<p align="center">
  <img src="assets/openclaw-finance.png" alt="OpenClaw-Finance" width="600"/>
</p>
  <p align="center">
...
## What is OpenClaw-Finance?

OpenClaw-Finance is an **AI financial agent** that connects to live market data, reasons through investment questions, and delivers analysis wherever you are: Telegram, Discord, Slack, email, or the command line.
...
OpenClaw-Finance can also **launch meme coins end-to-end**: scan Twitter and RSS feeds for viral ideas, evaluate meme potential with LLM scoring, and deploy tokens directly on **pump.fun (Solana)** or **four.meme (BSC)** — all from a single natural-language prompt.

<img src="assets/openclaw-finance-infographic.png" alt="OpenClaw-Finance Infographic" width="800"/>

## Why OpenClaw-Finance
...
```bash
openclaw-finance onboard
# Edit ~/.openclaw-finance/config.json to add your LLM API key
```
...
# Single question
openclaw-finance agent -m "Analyze AAPL's latest earnings and valuation"
...
```bash
# Build
docker build -t openclaw-finance .

# Gateway
docker run -d \
  -v ~/.openclaw-finance:/root/.openclaw-finance \
  -p 18790:18790 \
  openclaw-finance gateway

# One-off query
docker run --rm \
  -v ~/.openclaw-finance:/root/.openclaw-finance \
  openclaw-finance agent -m "What's the current US CPI trend?"
```

## Configuration

OpenClaw-Finance stores config at `~/.openclaw-finance/config.json`.
...
Run `openclaw-finance onboard` to generate starter config interactively.
...
Set these in `~/.openclaw-finance/config.json`:
...
OpenClaw-Finance can deploy a new token on **pump.fun (Solana)** or **four.meme (BSC)** from a single prompt.
...
2. **Confirm** — OpenClaw-Finance presents candidates and waits for your approval before proceeding.
...
| Platform | Chain | Fee | What OpenClaw-Finance does |
|----------|-------|-----|-------------------|
...
Wallet credentials go in `~/.openclaw-finance/config.json` under `tools.memeMonitor`:
...
> OpenClaw-Finance always confirms token details with you before signing. Never share your private key.

## Chat Apps

Talk to OpenClaw-Finance through Telegram, Discord, WhatsApp, Feishu, Mochat, DingTalk, Slack, Email, or QQ — anytime, anywhere.
...
**1. Ask OpenClaw-Finance to set up Mochat for you**

Simply send this message to OpenClaw-Finance (replace `xxx@xxx` with your real email):

```
Read https://raw.githubusercontent.com/HKUDS/MoChat/refs/heads/main/skills/nanobot/skill.md and register on MoChat. My Email account is xxx@xxx Bind me as your owner and DM me on MoChat.
```

OpenClaw-Finance will automatically register, configure `~/.openclaw-finance/config.json`, and connect to Mochat.
...
That's it — OpenClaw-Finance handles the rest!
...
If you prefer to configure manually, add the following to `~/.openclaw-finance/config.json`:
...
```bash
openclaw-finance channels login
...
> - `allowFrom`: Leave empty for public access, or add user openids to restrict. You can find openids in the OpenClaw-Finance logs when a user messages the bot.
...
Give OpenClaw-Finance its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `openclaw-finance-bot@gmail.com`)
...
```json
{
  "channels": {
    "email": {
      ...
      "imapPort": 993,
      "imapUsername": "openclaw-finance-bot@gmail.com",
      "imapPassword": "your-app-password",
      "smtpHost": "smtp.gmail.com",
      "smtpPort": 587,
      "smtpUsername": "openclaw-finance-bot@gmail.com",
      "smtpPassword": "your-app-password",
      "fromAddress": "openclaw-finance-bot@gmail.com",
...
<p align="center">
  <img src="assets/architecture.jpg" alt="OpenClaw-Finance Architecture" width="800"/>
</p>
