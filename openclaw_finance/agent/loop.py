...
from openclaw_finance.agent.tools.registry import ToolRegistry
from openclaw_finance.agent.tools.filesystem import ReadFileTool, WriteFileTool, EditFileTool, ListDirTool
from openclaw_finance.agent.tools.shell import ExecTool
from openclaw_finance.agent.tools.web import TavilySearchTool, BraveSearchTool, WebFetchTool
from openclaw_finance.agent.tools.message import MessageTool
from openclaw_finance.agent.tools.spawn import SpawnTool
from openclaw_finance.agent.tools.cron import CronTool
from openclaw_finance.agent.memory import MemoryStore
from openclaw_finance.agent.subagent import SubagentManager
from openclaw_finance.agent.financial.router import FinancialMetricsRouter, FinancialSearchRouter
from openclaw_finance.agent.financial.equity_valuation_router import EquityValuationRouter
from openclaw_finance.agent.financial.economics_router import EconomicsRouter
...
from openclaw_finance.agent.financial.prediction_market_router import PredictionMarketRouter
from openclaw_finance.agent.financial_tools.financial_news_tool import FinancialNewsTool
from openclaw_finance.session.manager import Session, SessionManager
...
class AgentLoop:
    ...
    def __init__(
        ...
        from openclaw_finance.config.schema import ExecToolConfig
        ...
        from openclaw_finance.agent.financial import (
    ...
    async def _connect_mcp(self) -> None:
        ...
        from openclaw_finance.agent.tools.mcp import connect_mcp_servers
    ...
    async def _process_message(self, msg: InboundMessage, session_key: str | None = None) -> OutboundMessage | None:
        ...
        if intent.is_financial:
            ...
            if not self.fin_profile.is_complete():
                from openclaw_finance.agent.financial.prompts import ONBOARDING_PROMPT
            ...
            if intent.intent_type == "macro_analysis":
                from openclaw_finance.agent.financial.prompts import MACRO_ROUTING_PROMPT
                finance_context_parts.append(MACRO_ROUTING_PROMPT)

            # Inject meme routing when intent is meme
            if intent.intent_type == "meme":
                from openclaw_finance.agent.financial.prompts import MEME_ROUTING_PROMPT
            ...
            if intent.intent_type == "prediction_market":
                from openclaw_finance.agent.financial.prompts import PREDICTION_ROUTING_PROMPT
            ...
            if cache_hits:
                from openclaw_finance.agent.financial.prompts import CACHE_REUSE_CONTEXT_PROMPT
                summaries = []
...
Respond with ONLY valid JSON, no markdown fences."""
        ...
        try:
            ...
            if fin_history_content and len(fin_history_content) > 10000:
                from openclaw_finance.agent.financial.prompts import FINANCIAL_CONSOLIDATION_PROMPT
