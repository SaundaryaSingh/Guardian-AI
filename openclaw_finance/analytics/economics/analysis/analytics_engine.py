...
import logging
from ..base.base_models import EconomicsBase, ValidationError, CalculationError, DataError
...
class StatisticalAnalyzer(EconomicsBase):
    ...
    def _analyze_seasonality(self, data: pd.Series) -> Dict[str, Any]:
        """Detect seasonality via seasonal decomposition (requires statsmodels)."""
        try:
            from statsmodels.tsa.seasonal import seasonal_decompose
            if len(data) < 8:
                return {"error": "Insufficient data for seasonal decomposition"}
            period = 4 if len(data) >= 8 else 2
            decomp = seasonal_decompose(data, model="additive", period=period, extrapolate_trend="freq")
            seasonal_vals = decomp.seasonal.dropna().tolist()
            return {
                "has_seasonality": max(abs(v) for v in seasonal_vals) > 0.01 if seasonal_vals else False,
                "seasonal_component": [round(float(v), 4) for v in seasonal_vals[:period]],
                "period": period,
            }
    ...
    def _analyze_trend(self, data: pd.Series) -> Dict[str, Any]:
        ...
        # Generate forecasts (default 4 periods ahead)
        _forecast_periods = 4
        forecast_x = np.arange(len(data), len(data) + _forecast_periods)
...
class ForecastingEngine(EconomicsBase):
    ...
    def calculate(self, *args, **kwargs) -> Dict[str, Any]:
        """Dispatch to simple_forecasting_methods via calculate interface."""
        ...
        return self.simple_forecasting_methods(data=data, forecast_periods=forecast_periods, methods=methods)
    ...
    def simple_forecasting_methods(self, data: pd.Series,
        ...
        # Forecast evaluation not implemented in source — skip gracefully
        evaluation = None
