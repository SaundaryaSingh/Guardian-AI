...
class ResidualIncomeModel(BaseValuationModel):
    ...
    def validate_inputs(self, **kwargs) -> bool:
        """Validate RI model inputs."""
        current_book_value = kwargs.get('current_book_value', 0)
        required_return = kwargs.get('required_return', 0)
        return current_book_value > 0 and 0 < required_return < 1
...
class EconomicValueAddedModel(BaseValuationModel):
    ...
    def validate_inputs(self, **kwargs) -> bool:
        """Validate EVA model inputs."""
        wacc = kwargs.get('wacc', 0)
        return 0 < wacc < 1

    def calculate(self, **kwargs) -> ValuationResult:
        """Calculate EVA-based firm valuation."""
        nopat = kwargs.get('nopat', 0)
        invested_capital = kwargs.get('invested_capital', 0)
        wacc = kwargs.get('wacc', 0)
        current_price = kwargs.get('current_price', 0)
        terminal_growth = kwargs.get('terminal_growth', 0.0)
        eva_value = self.calculate_eva(nopat, invested_capital, wacc)
        firm_result = self.eva_valuation(
            current_invested_capital=invested_capital,
            projected_evas=[eva_value],
            wacc=wacc,
            terminal_eva=eva_value,
            terminal_growth=terminal_growth,
        )
        return ValuationResult(
            method=self.valuation_method,
            intrinsic_value=firm_result['total_firm_value'],
            current_price=current_price,
            recommendation="BUY" if eva_value > 0 else "SELL",
            upside_downside=0.0,
            confidence_level="MEDIUM",
            assumptions={'nopat': nopat, 'invested_capital': invested_capital, 'wacc': wacc},
            calculation_details=firm_result,
        )

    def calculate_intrinsic_value(self, company_data: CompanyData, market_data: MarketData) -> float:
        """Calculate intrinsic value using EVA approach."""
        financial_data = company_data.financial_data
        operating_cashflow = financial_data.get('operating_cashflow', 0)
        nopat = operating_cashflow * (1 - 0.21)
        total_debt = financial_data.get('total_debt', 0)
        total_cash = financial_data.get('total_cash', 0)
        invested_capital = company_data.market_cap + total_debt - total_cash
        wacc = market_data.required_return * 0.85
        shares = company_data.shares_outstanding or 1
        eva_value = self.calculate_eva(nopat, invested_capital, wacc)
        firm_val = self.eva_valuation(invested_capital, [eva_value], wacc, eva_value, 0.025)
        return (firm_val['total_firm_value'] - total_debt + total_cash) / shares
