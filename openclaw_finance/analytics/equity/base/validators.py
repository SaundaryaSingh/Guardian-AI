...
Data validation and quality checks following CFA standards.
"""
...
class DCFValidator:
    ...
    def validate_fcf_inputs(cash_flows: List[float], discount_rate: float,
        ...
        if warnings:
            _warnings_module.warn("DCF: " + "; ".join(warnings), stacklevel=2)
    ...
    def validate_fcff_calculation_inputs(ebit: float, tax_rate: float, depreciation: float,
        ...
        if tax_rate > 0.5:
            _warnings_module.warn(f"Tax rate of {tax_rate:.2%} is very high", stacklevel=2)

        if capex > abs(ebit) * 2:
            _warnings_module.warn("Capital expenditures are very high relative to EBIT", stacklevel=2)
...
class MultiplesValidator:
    ...
    def validate_comparable_companies(comparables: List[Dict[str, Any]],
        ...
        if warnings:
            _warnings_module.warn("Comparables: " + "; ".join(warnings), stacklevel=2)
    ...
    def validate_multiple_values(multiples: Dict[str, float]) -> bool:
        ...
        if warnings:
            _warnings_module.warn("Multiples: " + "; ".join(warnings), stacklevel=2)
...
class ResidualIncomeValidator:
    ...
    def validate_ri_inputs(net_income: float, book_value: float,
        ...
        if warnings:
            _warnings_module.warn("Residual Income: " + "; ".join(warnings), stacklevel=2)
...
class CompanyDataValidator:
    ...
    def validate_data_freshness(company_data: CompanyData, max_age_days: int = 7) -> bool:
        ...
        if not company_data.last_updated:
            _warnings_module.warn("No timestamp available for data freshness check", stacklevel=2)
            return True

        age = datetime.now() - company_data.last_updated
        if age.days > max_age_days:
            _warnings_module.warn(
                f"Data is {age.days} days old (max recommended: {max_age_days} days)", stacklevel=2
            )
...
def validate_all_inputs(valuation_method: ValuationMethod, **kwargs) -> bool:
    ...
    if valuation_method == ValuationMethod.DDM_GORDON:
    ...
    elif valuation_method in (ValuationMethod.DDM_TWO_STAGE,
                              ValuationMethod.DDM_THREE_STAGE,
                              ValuationMethod.DDM_H_MODEL):
        dividends = kwargs.get('dividends', [kwargs.get('dividend', 0)])
        growth_rates = kwargs.get('growth_rates', [kwargs.get('growth_rate', 0)])
        return DDMValidator.validate_multistage_ddm_inputs(
            dividends,
            growth_rates,
            kwargs.get('required_return', 0),
            kwargs.get('terminal_growth', 0),
        )
