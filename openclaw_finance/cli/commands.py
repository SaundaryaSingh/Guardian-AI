"""CLI commands for openclaw-finance."""
...
from openclaw_finance import __version__, __logo__
from openclaw_finance.config.schema import Config

app = typer.Typer(
    name="openclaw-finance",
    help=f"{__logo__} openclaw-finance - Financial Analysis Expert",
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".openclaw-finance" / "history" / "cli_history"
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} OpenClaw-Finance[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} openclaw-finance v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """openclaw-finance - Financial Analysis Expert."""
    pass
...
@app.command()
def onboard():
    """Initialize openclaw-finance configuration and workspace."""
    from openclaw_finance.config.loader import get_config_path, load_config, save_config
    from openclaw_finance.config.schema import Config
    from openclaw_finance.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} openclaw-finance is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.openclaw-finance/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]openclaw-finance agent -m \"Hello!\"[/cyan]")
...
You are OpenClaw-Finance, a helpful financial analysis expert. Be concise, data-driven, and accurate.
...
This document describes the tools available to OpenClaw-Finance.
...
I am OpenClaw-Finance, a helpful financial analysis expert.
...
def _make_provider(config: Config):
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key):
        ...
        console.print("Set one in ~/.openclaw-finance/config.json under providers section")
        raise typer.Exit(1)
...
def gateway(
    ...
    """Start the openclaw-finance gateway."""
    from openclaw_finance.config.loader import load_config, get_data_dir
    ...
    from openclaw_finance.session.manager import SessionManager
    from openclaw_finance.cron.service import CronService
    from openclaw_finance.cron.types import CronJob
    from openclaw_finance.heartbeat.service import HeartbeatService
    ...
    console.print(f"{__logo__} Starting openclaw-finance gateway on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from openclaw_finance.bus.events import OutboundMessage
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show openclaw-finance runtime logs during chat"),
):
    ...
    if logs:
        logger.enable("openclaw_finance")
    else:
        logger.disable("openclaw_finance")
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]OpenClaw-Finance is thinking...[/dim]", spinner="dots")
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".openclaw-finance" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # openclaw_finance/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall openclaw-finance")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from openclaw_finance.config.loader import get_data_dir
    from openclaw_finance.cron.service import CronService
...
def cron_add(
    ...
    from openclaw_finance.config.loader import get_data_dir
    from openclaw_finance.cron.service import CronService
    from openclaw_finance.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} openclaw-finance Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from openclaw_finance.providers.registry import PROVIDERS
